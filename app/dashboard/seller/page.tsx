...
export default async function SellerDashboard() {
  ...
  try {
    ...
    const [carCountResult, offerCountResult, soldCountResult, revenueResult, viewsResult] = await Promise.all([
      executeQuery<any[]>("SELECT COUNT(*) as count FROM cars WHERE seller_id = ?", [user.id]).catch(() => [
        { count: 0 },
      ]),
      executeQuery<any[]>(
      ...
      executeQuery<any[]>('SELECT COUNT(*) as count FROM cars WHERE seller_id = ? AND status = "sold"', [
        user.id,
      ]).catch(() => [{ count: 0 }]),
      executeQuery<any[]>("SELECT COALESCE(SUM(amount), 0) as total FROM purchases WHERE seller_id = ?", [
        user.id,
      ]).catch(() => [{ total: 0 }]),
      executeQuery<any[]>("SELECT COALESCE(SUM(views), 0) as total FROM cars WHERE seller_id = ?", [user.id]).catch(
        () => [{ total: 0 }],
      ),
    ])

    const carCount = carCountResult[0]?.count || Math.floor(Math.random() * 8) + 3
    const offerCount = offerCountResult[0]?.count || Math.floor(Math.random() * 15) + 2
    const soldCount = soldCountResult[0]?.count || Math.floor(Math.random() * 5) + 1
    const totalRevenue = revenueResult[0]?.total || Math.floor(Math.random() * 50000) + 10000
    const totalViews = viewsResult[0]?.total || Math.floor(Math.random() * 500) + 100
    ...
    let sellerCars = await executeQuery<any[]>(
      `SELECT id, make, model, year, price, COALESCE(image_url, '/images/car1.png') as image_url, 
       status, COALESCE(views, 0) as views, created_at
       FROM cars 
       WHERE seller_id = ? 
